@props([
    'profile'   => new \App\Models\Education\Profile(),
    'place'     => new \App\Models\Education\Place(),
])
    ...
    <a href="#" class="p-4">
        <div>
            ...
            <h2 class="text-xl font-[600] name mb-6">
                {{ $profile->speciality->spec_code }}
                -
                {{ $profile->speciality->name }}

                @if( $profile->speciality->name_profile )
                    ({{ $profile->speciality->name_profile}})
                @endif
            </h2>
        </div>
