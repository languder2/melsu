<div class="content_section mt-4 bg-white p-4">
    <div class="flex gap-4 items-center">
        <div class="flex-1">
            <x-form.input
                ...
                name="test"
                name="partner_sections[{{$id??(int)microtime(true)}}][title]"
                value="{{ old('partner_sections.{$id}.title') ?? $section->title ?? null}}"
        ...
        <div>
            <a
                href="{{ route('api:partner:sections:delete', ['id' => $section->id, '_token' => csrf_token()]) }}"
    ...
    <div class="flex gap-4 items-center mt-3">
        <div class="flex-1">
            <x-form.input
                ...
                name="partner_sections[{{$id??(int)microtime(true)}}][order]"
                type="number"
        ...
        <div class="flex-1">
            <x-form.checkbox.base
                ...
                name="partner_sections[{{$id??(int)microtime(true)}}][show_title]"
                text="Выводить заголовок"
                class="text-lg"
                :checked="
                    old('_token')
                    ? old('partner_sections.{$id}.title')
        ...
        <div class="flex-1">
            <x-form.checkbox.base
                ...
                name="partner_sections[{{$id??(int)microtime(true)}}][show]"
    ...
    >
        <x-form.editor
            ...
            name="partner_sections[{{$id??(int)microtime(true)}}][content]"
            label="Контент"
            value="{{old('partner_sections.{$id}.content') ?? $section->content ?? null}}"
        />
    </div>
</div>
