<div class="content_section mt-4 bg-white p-4">
    <div class="flex gap-4 items-center">
        <div class="flex-1">
            <x-form.input
                ...
                name="test"
                name="upbringing_sections[{{$id??(int)microtime(true)}}][title]"
                value="{{ old('upbringing_sections.{$id}.title') ?? $section->title ?? null}}"
        ...
        <div>
            <a
                href="{{ route('api:upbringing:sections:delete', ['id' => $section->id, '_token' => csrf_token()]) }}"
    ...
    <div class="flex gap-4 items-center mt-3">
        <div class="flex-1">
            <x-form.input
                ...
                name="upbringing_sections[{{$id??(int)microtime(true)}}][order]"
                type="number"
        ...
        <div class="flex-1">
            <x-form.checkbox.base
                ...
                name="upbringing_sections[{{$id??(int)microtime(true)}}][show_title]"
                text="Выводить заголовок"
                class="text-lg"
                :checked="
                    old('_token')
                    ? old('upbringing_sections.{$id}.title')
        ...
        <div class="flex-1">
            <x-form.checkbox.base
                ...
                name="upbringing_sections[{{$id??(int)microtime(true)}}][show]"
    ...
    >
        <x-form.editor
            ...
            name="upbringing_sections[{{$id??(int)microtime(true)}}][content]"
            label="Контент"
            value="{{old('upbringing_sections.{$id}.content') ?? $section->content ?? null}}"
        />
    </div>
</div>
