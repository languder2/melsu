...
>
    <div class="flex-1">
        ...
        <x-form.input
            ...
            value="{{ old('_token') ? old('documents.'.$item->id.'.title') : $item->title }}"
        />
        ...
        <div class="grid grid-cols-2 gap-4 ">
            <div>
                <x-form.select2
                    id="category_id"
                    name="documents[{{ $item->id }}][form]"
                    :value=" old('_token') ? old('documents.'.$item->id.'.form') : $item->SpecialityForm ?? null  "
                    null="Документ к специальности {!! $item->type !!}"
                    :list="\App\Enums\EducationForm::getList()"
                />
            </div>
            ...
            <div>
                <x-form.select2
                    id="category_id"
                    name="documents[{{ $item->id }}][type]"
                    :value=" old('_token') ? old('documents.'.$item->id.'.type') : $item->type ?? null  "
                    null="Категория"
                    :list="\App\Enums\DocumentSpecialityType::getList()"
                />
            </div>
        </div>
