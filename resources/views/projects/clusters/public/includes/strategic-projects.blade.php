<div
    class="
        bg-black
        -mb-8
    "
>
    <div class="container text-white pt-20 pb-14 flex flex-col lg:flex-row gap-4 justify-between">
        <div class="flex-1 flex flex-col gap-4 lg:max-w-1/3">
            <h3 class="font-semibold text-3xl uppercase">
                Системообразующие проекты
            </h3>
            <div class="font-semibold text-xl">
                Вместе мы создаем будущее!
            </div>
            <div class="font-semibold">
                Мелитопольский государственный университет приглашает к сотрудничеству всех заинтересованных лиц для совместной реализации флагманских проектов и внесения вклада в развитие региона и страны. Следите за новостями на нашем сайте и присоединяйтесь к нашим инициативам!
            </div>
        </div>
        <a href="{{ url('notsmu') }}"
            class="flex flex-col gap-3 border border-white p-6 w-full lg:w-100 h-100 bg-center bg-cover hover:bg-red-800"
            style="background-image: url('{{asset('img/clusters/Component 20.svg')}}')"
        >
            <h3 class="font-semibold text-lg" >
                НОЦ мирового уровня «НовоРоссия – форпост русских технологий»
            </h3>
            <p class="">
                При активном участии ФГБОУ «Мелитопольский государственный университет» подготовлен проект по созданию в нашем регионе научно-образовательного центра мирового уровня «НовоРоссия — форпост русских технологий».
            </p>
        </a>

    </div>
</div>
