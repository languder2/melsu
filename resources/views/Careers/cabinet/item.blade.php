...
>
    ...
    <div class="p-3 bg-white rounded-sm shadow flex items-center justify-center">
        @if($item->salary_from)
            {{ __('common.from') . ' ' . number_format($item->salary_from) . __('common.currencyRub') }}
        @endif

        @if($item->salary_from)
            {{ __('common.to') . ' ' . number_format($item->salary_to) . __('common.currencyRub') }}
        @endif
    </div>
